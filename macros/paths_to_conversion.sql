...
{%- set __, last_processed_cv_tstamp = snowplow_utils.return_limits_from_model(this,'cv_path_start_tstamp','cv_path_start_tstamp',true) %}
  ...
  with paths as (
    ...
    {% if var('snowplow__consider_intrasession_channels') %}
      ...
      and ((mkt_medium is not null and mkt_medium != '') or view_in_session_index = 1 or {{ var('snowplow__consider_all_page_views',false) }})
    {% endif %}
  ...
  , string_aggs as (
    ...
    select
      ...
      {% for grouping in var('snowplow__attribution_groupings') %}
        {{ snowplow_utils.get_string_agg(grouping, 'p', separator=' > ', sort_numeric=false, order_by_column='visit_start_tstamp', order_by_column_prefix='p') }} as {{ grouping }},
      {% endfor %}
  ...
  , arrays as (
    ...
    select
      ...
      {% for grouping in var('snowplow__attribution_groupings') %}
        {{ snowplow_utils.get_split_to_array(grouping, 's', ' > ') }} as {{grouping}}_path,
        {{ snowplow_utils.get_split_to_array(grouping, 's', ' > ') }} as {{grouping}}_transformed_path,
      {% endfor %}
  ...
  select
    ...
    {% for grouping in var('snowplow__attribution_groupings') %}
      {{ snowplow_utils.get_array_to_string(grouping + '_path', 't', ' > ') }} as {{grouping}}_path,
      {{ snowplow_utils.get_array_to_string(grouping + '_transformed_path', 't', ' > ') }} as {{grouping}}_transformed_path,
    {% endfor %}
