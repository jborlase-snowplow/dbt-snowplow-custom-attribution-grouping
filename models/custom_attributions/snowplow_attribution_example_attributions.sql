...
-- Create a custom attribution model that uses custom groupings
-- Simply copy this model, rename it, and change the `grouping` variable to the desired grouping
-- Ensure that the grouping is present in the `snowplow__attributions_groupings` variable
{% set grouping = "medium" %}
...
with arrays as (
  ...
  select
    ...
    c.{{grouping}}_transformed_path,
    {{ snowplow_utils.get_split_to_array(grouping+'_transformed_path', 'c', ' > ') }} as {{grouping}}_path_array
...
, unnesting as (
  {{ snowplow_utils.unnest('cv_id', grouping+'_path_array', grouping, 'arrays', with_index=true) }}
)
...
, prep as (
  ...
  select 
    ...
    a.cv_type,
    a.revenue,
    a.{{grouping}}_transformed_path,
    cast(u.{{grouping}} as {{ dbt.type_string() }}) as {{grouping}},
    u.source_index,
    {{ snowplow_utils.get_array_size(grouping+'_path_array') }} as path_length,
    case when u.source_index = max(u.source_index) over (partition by u.cv_id) then true else false end as is_last_element,
    case when {{ snowplow_utils.get_array_size(grouping+'_path_array') }} = 1 then revenue
        when {{ snowplow_utils.get_array_size(grouping+'_path_array') }} = 2 then revenue/2
...
select
  cv_id || {{grouping}} || source_index as composite_key,
  cv_id,
  event_id,
  customer_id,
  cv_tstamp,
